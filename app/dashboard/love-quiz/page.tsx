...
import { Plus, X, Check, ChevronRight, RefreshCw, Trophy } from 'lucide-react'
...
export default function LoveQuizPage() {
  ...
  const [quizAsker, setQuizAsker] = useState('')
  ...
  async function loadData() {
    ...
    if (pRes.data) {
      ...
      setQuizAsker(pRes.data.person2_name)
    }
    setLoading(false)
  }
  ...
  function startQuiz() {
    const pool = questions.filter(q => filterAsker === 'all' ? true : q.asked_by !== quizAsker)
  ...
  return (
    <div style={{ maxWidth: '700px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {questions.length > 0 && !quizMode && (
            <button onClick={startQuiz} style={{ background: 'linear-gradient(135deg, #a855f7, #ec4899)', color: '#fff', border: 'none', borderRadius: '50px', padding: '10px 18px', cursor: 'pointer', fontWeight: 700, fontFamily: 'Lato,sans-serif', fontSize: '0.875rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <ChevronRight size={16} /> Mulai Quiz!
            </button>
          )}
          <button onClick={() => setShowAdd(true)} className="btn-rose" style={{ gap: '6px', fontSize: '0.875rem', padding: '10px 18px' }}>
            <Plus size={16} /> + Pertanyaan
          </button>
        </div>
      </div>
      ...
      {quizMode && !quizDone && session.length > 0 && (
        <div>
          ...
          <div className="glass" style={{ borderRadius: '24px', border: '1px solid #fecdd3', overflow: 'hidden' }}>
            <div style={{ background: 'linear-gradient(135deg, #f43f5e, #ec4899)', padding: '24px', textAlign: 'center' }}>
              <span style={{ background: 'rgba(255,255,255,0.2)', borderRadius: '50px', padding: '4px 12px', fontSize: '0.75rem', color: '#fff', fontFamily: 'Lato,sans-serif' }}>
              ...
              <p className="font-display" style={{ color: '#fff', fontSize: '1.1rem', fontWeight: 600, marginTop: '12px', marginBottom: 0, lineHeight: 1.4 }}>
                {session[currentIdx].question.question}
              </p>
              <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.75rem', marginTop: '8px', fontFamily: 'Lato,sans-serif' }}>
            ...
            <div style={{ padding: '24px' }}>
              {!showAnswer ? (
                <>
                  ...
                  />
                  <button onClick={submitAnswer} disabled={!userInput.trim()} className="btn-rose" style={{ width: '100%', justifyContent: 'center' }}>
                    Cek Jawaban →
                  </button>
                </>
              ) : (
                <div>
                  <div style={{ borderRadius: '16px', padding: '16px', marginBottom: '16px', textAlign: 'center', background: session[currentIdx].isCorrect ? '#f0fdf4' : '#fef2f2', border: `2px solid ${session[currentIdx].isCorrect ? '#86efac' : '#fca5a5'}` }}>
                    <div style={{ fontSize: '2.5rem', marginBottom: '8px' }}>{session[currentIdx].isCorrect ? '🎉' : '😅'}</div>
                    <p className="font-body" style={{ fontWeight: 700, color: session[currentIdx].isCorrect ? '#15803d' : '#dc2626', fontSize: '0.95rem', margin: '0 0 4px' }}>
                      {session[currentIdx].isCorrect ? 'Benar! Kamu kenal aku 💕' : 'Belum tepat 😢'}
                    </p>
                    {!session[currentIdx].isCorrect && (
                      <p className="font-body" style={{ color: '#6b7280', fontSize: '0.85rem', margin: '8px 0 0' }}>
                        Jawaban: <strong style={{ color: '#9f1239' }}>{session[currentIdx].question.answer}</strong>
                      </p>
                    )}
                  </div>
      ...
      {/* Quiz Result */}
      {quizMode && quizDone && (
        <div className="glass" style={{ borderRadius: '24px', border: '1px solid #fecdd3', overflow: 'hidden', textAlign: 'center' }}>
          <div style={{ background: 'linear-gradient(135deg, #f43f5e, #ec4899)', padding: '32px' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.8)', fontFamily: 'Lato,sans-serif', marginTop: '6px' }}>
      ...
      {!quizMode && (
        <div>
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
            {[['all','Semua'], ...(authorOptions.map(n => [n, n]))].map(([v, l]) => (
          ...
          {loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {filteredQ.map(q => {
                ...
                return (
                  <div key={q.id} className="glass" style={{ borderRadius: '14px', padding: '14px 16px', border: '1px solid #fecdd3', display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ fontSize: '1.5rem' }}>{cat?.emoji || '🌟'}</div>
                    <div style={{ flex: 1 }}>
                      <p className="font-body" style={{ fontWeight: 600, color: '#9f1239', fontSize: '0.88rem', margin: 0 }}>{q.question}</p>
                      <p className="font-body" style={{ color: '#fda4af', fontSize: '0.72rem', margin: '3px 0 0' }}>
                        {isP1 ? '👩' : '👨'} Dari {q.asked_by} · {cat?.label}
                      </p>
                    </div>
                    <div style={{ background: '#fff1f2', borderRadius: '8px', padding: '4px 10px' }}>
                      <span className="font-body" style={{ color: '#e11d48', fontSize: '0.75rem', fontWeight: 600 }}>{q.answer}</span>
                    </div>
                    <button onClick={() => deleteQuestion(q.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#fda4af', display: 'flex', padding: '4px' }}>
                      <X size={14} />
                    </button>
                  </div>
                )
          ...
          <div style={{ background: '#fff', borderRadius: '24px', padding: '24px', width: '100%', maxWidth: '460px', boxShadow: '0 25px 60px rgba(0,0,0,0.2)', maxHeight: '90vh', overflowY: 'auto' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                {SAMPLE_QUESTIONS.slice(0,4).map(s => (
                  <button key={s} onClick={() => setForm(f => ({ ...f, question: s }))}
                    style={{ background: '#fff1f2', border: '1px solid #fecdd3', borderRadius: '8px', padding: '4px 10px', fontSize: '0.72rem', color: '#be123c', cursor: 'pointer', fontFamily: 'Lato,sans-serif' }}>
                    {s}
                  </button>
                ))}
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div>
                ...
                <input className="love-input" placeholder="Contoh: Makanan favorit aku apa?" value={form.question} onChange={e => setForm(f => ({ ...f, question: e.target.value }))} />
              </div>
              <div>
                <label className="font-body" style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#be123c', marginBottom: '6px' }}>Jawaban *</label>
                <input className="love-input" placeholder="Jawaban yang benar..." value={form.answer} onChange={e => setForm(f => ({ ...f, answer: e.target.value }))} />
              </div>
              <div>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button onClick={() => setShowAdd(false)} className="font-body" style={{ flex: 1, padding: '12px', borderRadius: '12px', border: '2px solid #fecdd3', background: '#fff', color: '#fb7185', fontWeight: 600, cursor: 'pointer' }}>Batal</button>
