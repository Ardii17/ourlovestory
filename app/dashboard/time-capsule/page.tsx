  ...
  return (
    <div style={{ maxWidth: '700px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          <h1 className="font-display" style={{ fontSize: '1.5rem', fontWeight: 700, color: '#1a5c47', margin: 0 }}>Time Capsule ⏳</h1>
          <p className="font-body" style={{ color: '#5bb89a', fontSize: '0.85rem', marginTop: '4px' }}>Pesan rahasia untuk dibuka di masa depan</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="glass" style={{ borderRadius: '24px', padding: '60px 32px', textAlign: 'center', border: '1px solid #c8ddd5' }}>
          <div style={{ fontSize: '4rem', marginBottom: '12px' }}>⏳</div>
          <h3 className="font-display" style={{ color: '#1a5c47', fontSize: '1.2rem', marginBottom: '8px' }}>Belum ada kapsul waktu</h3>
          <p className="font-body" style={{ color: '#5bb89a', fontSize: '0.85rem', marginBottom: '20px' }}>Tuliskan pesan untuk dibuka di masa depan!</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          {locked.length > 0 && (
            <div>
              <h2 className="font-display" style={{ color: '#1a5c47', fontSize: '1rem', fontWeight: 700, marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <Lock size={16} color="#2d8c6e" /> Terkunci ({locked.length})
              </h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {locked.map(c => (
                  <div key={c.id} className="glass" style={{ borderRadius: '16px', border: '1px solid #c8ddd5', overflow: 'hidden' }}>
                    <div style={{ background: 'linear-gradient(90deg, #2d8c6e08, #e8943a08)', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: '16px' }}>
                      <div style={{ fontSize: '2.5rem' }}>🔒</div>
                      <div style={{ flex: 1 }}>
                        <h3 className="font-display" style={{ fontWeight: 700, color: '#1a5c47', fontSize: '0.95rem', margin: 0 }}>{c.title}</h3>
                        <p className="font-body" style={{ color: '#5bb89a', fontSize: '0.78rem', margin: '2px 0 0' }}>
                          Dari {c.from_person} → {c.to_person}
                        </p>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <div className="font-display" style={{ fontSize: '1.5rem', fontWeight: 700, color: '#2d8c6e' }}>{daysLeft(c.open_date)}</div>
                        <div className="font-body" style={{ fontSize: '0.7rem', color: '#a0c4b8' }}>hari lagi</div>
                        <div className="font-body" style={{ fontSize: '0.7rem', color: '#a0c4b8', marginTop: '2px' }}>
                          {format(parseISO(c.open_date), 'd MMM yyyy', { locale: idLocale })}
                        </div>
                      </div>
                      <button onClick={() => deleteCapsule(c.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#a0c4b8', padding: '4px', display: 'flex' }}>
                        <X size={14} />
                      </button>
                    </div>
                    {/* progress bar */}
                    <div style={{ height: '4px', background: '#e3f0eb' }}>
                      <div style={{
                        height: '4px',
                        background: 'linear-gradient(90deg, #2d8c6e, #e8943a)',
          ...
          {opened.length > 0 && (
            <div>
              <h2 className="font-display" style={{ color: '#1a5c47', fontSize: '1rem', fontWeight: 700, marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                ✉️ Sudah Dibuka ({opened.length})
              </h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {opened.map(c => (
                  <div key={c.id} className="glass" style={{ borderRadius: '16px', border: '1px solid #c8ddd5', cursor: 'pointer' }}
                    onClick={() => { setSelectedCapsule(c); setShowReveal(true) }}>
                    <div style={{ padding: '14px 20px', display: 'flex', alignItems: 'center', gap: '14px' }}>
                      <div style={{ fontSize: '2rem' }}>✉️</div>
                      <div style={{ flex: 1 }}>
                        <h3 className="font-display" style={{ fontWeight: 700, color: '#1a5c47', fontSize: '0.9rem', margin: 0 }}>{c.title}</h3>
                        <p className="font-body" style={{ color: '#a0c4b8', fontSize: '0.75rem', margin: '2px 0 0' }}>
                          {c.from_person} → {c.to_person} · {format(parseISO(c.open_date), 'd MMM yyyy', { locale: idLocale })}
                        </p>
                      </div>
                      <button onClick={e => { e.stopPropagation(); deleteCapsule(c.id) }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#a0c4b8', padding: '4px', display: 'flex' }}>
                        <X size={14} />
                      </button>
                    </div>
                  </div>
      ...
      {showReveal && selectedCapsule && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(6px)' }}
          ...
          <div style={{ background: '#fffdf7', borderRadius: '24px', width: '100%', maxWidth: '460px', overflow: 'hidden', boxShadow: '0 30px 80px rgba(0,0,0,0.3)' }}
            ...
            <div style={{ background: 'linear-gradient(135deg, #2d8c6e, #e8943a)', padding: '28px', textAlign: 'center' }}>
            ...
            <div style={{ padding: '28px' }}>
              <div className="paper-texture" style={{ borderRadius: '16px', padding: '20px', marginBottom: '20px' }}>
                <p className="font-body" style={{ color: '#1a5c47', lineHeight: 1.8, whiteSpace: 'pre-wrap', margin: 0 }}>{selectedCapsule.message}</p>
              </div>
              <p className="font-body" style={{ color: '#a0c4b8', fontSize: '0.75rem', textAlign: 'center', marginBottom: '16px' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)' }}>
          <div style={{ background: '#fff', borderRadius: '24px', padding: '24px', width: '100%', maxWidth: '460px', boxShadow: '0 25px 60px rgba(0,0,0,0.2)', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
              <h2 className="font-display" style={{ fontSize: '1.2rem', fontWeight: 700, color: '#1a5c47', margin: 0 }}>Buat Kapsul Waktu ⏳</h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div>
                <label className="font-body" style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#1a5c47', marginBottom: '6px' }}>Judul Kapsul *</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                <div>
                  <label className="font-body" style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#1a5c47', marginBottom: '6px' }}>Dari</label>
                ...
                <div>
                  <label className="font-body" style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#1a5c47', marginBottom: '6px' }}>Untuk</label>
              ...
              <div>
                <label className="font-body" style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#1a5c47', marginBottom: '6px' }}>Tanggal Bisa Dibuka *</label>
                <input type="date" className="love-input" value={form.open_date}
                  min={new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().split('T')[0]}
                  onChange={e => setForm(p => ({ ...p, open_date: e.target.value }))} />
                <p className="font-body" style={{ color: '#a0c4b8', fontSize: '0.72rem', marginTop: '4px' }}>
                  ⏳ {daysLeft(form.open_date)} hari lagi dari sekarang
                </p>
              </div>
              <div>
                <label className="font-body" style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#1a5c47', marginBottom: '6px' }}>Isi Pesan *</label>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button onClick={() => setShowModal(false)} className="font-body" style={{ flex: 1, padding: '12px', borderRadius: '12px', border: '2px solid #c8ddd5', background: '#fff', color: '#5bb89a', fontWeight: 600, cursor: 'pointer' }}>Batal</button>
