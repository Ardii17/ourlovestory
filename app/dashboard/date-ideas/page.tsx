...
const BUILT_IN: BuiltInIdea[] = [
  { id: 1,  emoji: '🌅', title: 'Nonton Sunset Berdua',      desc: 'Cari bukit atau pantai terdekat, bawa tikar dan camilan favorit kalian. Duduk bersantai sambil menikmati matahari terbenam.',          category: 'outdoor',  budget: 'gratis',   duration: '2-3 jam',  vibe: ['tenang','damai'],      source: 'builtin' },
  ...
  { id: 3,  emoji: '🎬', title: 'Marathon Film Seharian',    desc: 'Buat daftar film yang ingin ditonton berdua, siapkan popcorn dan selimut, dan nikmati waktu santai yang menyenangkan.',                 category: 'indoor',   budget: 'gratis',   duration: 'seharian', vibe: ['santai','damai'],      source: 'builtin' },
  ...
  { id: 6,  emoji: '⭐', title: 'Stargazing Malam Hari',     desc: 'Pergi ke tempat yang jauh dari lampu kota, bawa selimut tebal dan cokelat panas, bersantai sambil melihat bintang.',                     category: 'outdoor',  budget: 'gratis',   duration: '2-3 jam',  vibe: ['tenang','damai'],      source: 'builtin' },
  ...
  { id: 11, emoji: '💆', title: 'Spa Day di Rumah',          desc: 'Beli masker wajah, bath bomb, dan produk perawatan. Manjakan diri dengan spa rumahan yang menenangkan.',                         category: 'indoor',   budget: 'murah',    duration: '2-3 jam',  vibe: ['santai','damai'],      source: 'builtin' },
  { id: 12, emoji: '🌃', title: 'City Walk Malam Hari',      desc: 'Jalan kaki atau naik motor keliling kota di malam hari, cari jajanan malam favorit, dan nikmati lampu kota bersama.',                category: 'outdoor',  budget: 'gratis',   duration: '2-3 jam',  vibe: ['seru','petualang'],   source: 'builtin' },
  ...
  { id: 16, emoji: '📸', title: 'Sesi Foto Berdua',          desc: 'Pilih lokasi cantik di kota kalian, pakai outfit favorit, dan foto-foto berdua. Bisa jadi kenangan abadi!',                         category: 'outdoor',  budget: 'gratis',   duration: '2-3 jam',  vibe: ['berkesan','seru'],        source: 'builtin' },
  { id: 17, emoji: '🍽️', title: 'Fine Dining Special',       desc: 'Booking restoran favorit atau baru yang ingin dicoba. Berpakaian rapi dan nikmati momen istimewa ini.',                       category: 'outdoor',  budget: 'premium',  duration: '2-3 jam',  vibe: ['berkesan','mewah'],       source: 'builtin' },
  ...
  { id: 21, emoji: '🌄', title: 'Camping Dadakan',           desc: 'Setup tenda di halaman atau rooftop, masak sosis di atas api kecil, dan tidur di bawah bintang bersama.',                           category: 'outdoor',  budget: 'murah',    duration: 'semalam',  vibe: ['petualang','seru'],   source: 'builtin' },
  ...
  { id: 24, emoji: '🛁', title: 'Bubble Bath Santai',      desc: 'Siapkan bath bomb, lilin aromaterapi, dan musik lembut. Manjakan diri dengan relaksasi yang menenangkan.',                    category: 'indoor',   budget: 'murah',    duration: '1-2 jam',  vibe: ['damai','santai'],      source: 'builtin' },
]
...
const VIBE_COLOR: Record<string, string> = {
  damai: '#2d8c6e', seru: '#f59e0b', santai: '#3b82f6',
  petualang: '#22c55e', tenang: '#8b5cf6', hangat: '#f97316',
  kreatif: '#e8943a', manis: '#cc7a28', kompetitif: '#ef4444',
...
const BUDGET_COLOR: Record<string, string> = {
  gratis: '#22c55e', murah: '#3b82f6', menengah: '#f59e0b', premium: '#e8943a',
}
...
function placeToIdea(p: WishlistPlace): WishlistIdea {
  ...
  const vibeMap: Record<string, string[]> = {
    cafe: ['santai','hangat'], restaurant: ['kuliner','berkesan'],
    hotel: ['mewah','berkesan'], pantai: ['segar','petualang'],
...
export default function DateIdeasPage() {
  ...
  return (
    <div style={{ maxWidth: '680px', margin: '0 auto' }}>
      ...
      {/* Source filter */}
      <div className="glass" style={{ borderRadius: '16px', padding: '14px 16px', border: '1px solid #c8ddd5', marginBottom: '16px' }}>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                borderRadius: '12px',
                border: '2px solid',
                borderColor: sourceFilter === v ? '#2d8c6e' : '#c8ddd5',
                background: sourceFilter === v ? 'linear-gradient(135deg,#f4f9f7,#e3f0eb)' : '#fff',
                cursor: 'pointer',
                textAlign: 'left',
                transition: 'all 0.2s',
              }}
            >
              <div className="font-body" style={{ fontWeight: 700, color: sourceFilter === v ? '#237a5e' : '#1a5c47', fontSize: '0.82rem' }}>{l}</div>
              <div className="font-body" style={{ color: sourceFilter === v ? '#5bb89a' : '#a0c4b8', fontSize: '0.7rem', marginTop: '2px' }}>{sub}</div>
            </button>
          ))}
        </div>
      ...
      {sourceFilter === 'wishlist' && wishlist.length > 0 && (
        <div className="glass" style={{ borderRadius: '14px', border: '1px solid #c8ddd5', marginBottom: '16px', overflow: 'hidden' }}>
          <div style={{ padding: '10px 14px', borderBottom: '1px solid #e3f0eb', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <List size={14} color="#5bb89a" />
            <span className="font-body" style={{ color: '#1a5c47', fontSize: '0.78rem', fontWeight: 700 }}>
            ...
            {wishlist.map(p => (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 8px', borderRadius: '8px' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p className="font-body" style={{ fontWeight: 600, color: '#1a5c47', fontSize: '0.8rem', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</p>
                  {p.address && (
                    <p className="font-body" style={{ color: '#a0c4b8', fontSize: '0.68rem', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>📍 {p.address}</p>
                  )}
                </div>
                <span style={{ background: '#f4f9f7', borderRadius: '6px', padding: '2px 7px', fontSize: '0.65rem', color: '#5bb89a', fontFamily: 'Lato,sans-serif', fontWeight: 600, flexShrink: 0 }}>
                  {p.category}
                </span>
              </div>
            ))}
      ...
      <div style={{ marginBottom: '16px' }}>
        {!current ? (
          <div className="glass" style={{ borderRadius: '24px', border: '2px dashed #c8ddd5', padding: '52px 32px', textAlign: 'center' }}>
            <div style={{ fontSize: '4rem', marginBottom: '14px' }}>🎲</div>
            <h3 className="font-display" style={{ color: '#1a5c47', fontSize: '1.2rem', marginBottom: '6px' }}>Tekan tombol di bawah!</h3>
            <p className="font-body" style={{ color: '#a0c4b8', fontSize: '0.85rem', margin: 0 }}>
          ...
          <div className="glass" style={{ borderRadius: '24px', border: `1px solid ${current.source === 'wishlist' ? '#86efac' : '#c8ddd5'}`, overflow: 'hidden' }}>
            {/* Badge sumber */}
            <div style={{
            ...
            <div style={{ padding: '20px' }}>
              <p className="font-body" style={{ color: '#1a5c47', lineHeight: 1.7, marginBottom: '14px', fontSize: '0.9rem' }}>
                {current.desc}
              </p>

              {/* Info chips */}
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px', background: '#f4f9f7', borderRadius: '50px', padding: '5px 12px' }}>
                  <Clock size={12} color="#5bb89a" />
                  <span className="font-body" style={{ fontSize: '0.72rem', color: '#1a5c47', fontWeight: 600 }}>{current.duration}</span>
                </div>
                {'budget' in current && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px', background: '#f4f9f7', borderRadius: '50px', padding: '5px 12px' }}>
                    <Wallet size={12} color="#5bb89a" />
                    <span className="font-body" style={{ fontSize: '0.72rem', fontWeight: 600, color: BUDGET_COLOR[current.budget] || '#5bb89a' }}>
                      {(current.budget as string).charAt(0).toUpperCase() + (current.budget as string).slice(1)}
                    </span>
                  </div>
                )}
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px', background: '#f4f9f7', borderRadius: '50px', padding: '5px 12px' }}>
                  <MapPin size={12} color="#5bb89a" />
                  <span className="font-body" style={{ fontSize: '0.72rem', color: '#1a5c47', fontWeight: 600 }}>
              ...
              <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                {current.vibe.map(v => (
                  <span key={v} style={{ background: VIBE_COLOR[v] || '#2d8c6e', color: '#fff', borderRadius: '50px', padding: '3px 10px', fontSize: '0.7rem', fontWeight: 700, fontFamily: 'Lato,sans-serif' }}>
                    {v}
                  </span>
                ))}
              </div>
      ...
      <button
        ...
        style={{
          ...
          boxShadow: isSpinning || pool.length === 0 ? 'none' : '0 8px 30px rgba(45,140,110,0.35)',
      ...
      {history.length > 1 && (
        <div>
          <h3 className="font-display" style={{ color: '#1a5c47', fontSize: '0.9rem', fontWeight: 700, marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <Sparkles size={14} color="#5bb89a" /> Riwayat Pilihan
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
            {history.slice(1).map(h => (
              <div key={String(h.id)} className="glass" style={{ borderRadius: '12px', padding: '10px 14px', border: '1px solid #c8ddd5', display: 'flex', alignItems: 'center', gap: '10px', opacity: 0.65 }}>
                <span style={{ fontSize: '1.4rem', flexShrink: 0 }}>{h.emoji}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p className="font-body" style={{ fontWeight: 600, color: '#1a5c47', fontSize: '0.82rem', margin: '0 0 1px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{h.title}</p>
                  <p className="font-body" style={{ color: '#a0c4b8', fontSize: '0.68rem', margin: 0 }}>
