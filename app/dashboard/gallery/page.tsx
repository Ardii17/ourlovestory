...
export default function GalleryPage() {
  ...
  const [filterType, setFilterType] = useState<
    "all" | "profile" | "memory" | "documentation"
  >("all");
  ...
  const [uploadTarget, setUploadTarget] = useState<"documentation" | "memory">(
    "documentation",
  );
  ...
  async function handleMediaUpload(file: File) {
    ...
    const bucketName =
      uploadTarget === "documentation" ? "place-photos" : "memory-photos";
    const prefix =
      uploadTarget === "documentation" ? `place_${selectedPlaceId}` : `memory`;
      ...
      const [profileRes, memoriesRes, photosRes, placesRes] = await Promise.all(
        [
          ...
          supabase
            .from("memories")
            .select("*")
            .is("photo_url", "not.null")
            .order("memory_date", { ascending: false }),
          supabase
            .from("place_photos")
            .select("*, places(name, visited_date, category)")
            .order("created_at", { ascending: false }),
          supabase
            .from("places")
            .select("id, name, category, visited_date")
            .eq("status", "visited")
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex flex-col items-start justify-between gap-4 mb-8 md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-rose-500 font-body">
            Lihat semua lembaran foto dan video perjalanan cinta kita dalam satu
            halaman indah
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-xs font-bold text-white transition-colors rounded-full shadow-md cursor-pointer bg-[#2d8c6e] hover:bg-rose-600 shadow-rose-200"
          >
          ...
          <div className="flex items-center gap-4 px-4 py-2 text-xs font-semibold border rounded-full glass border-rose-100 text-rose-700">
            <div>
              📸 {items.filter((i) => i.type === "documentation").length}{" "}
              Dokumentasi
            </div>
            <div className="w-[1px] h-4 bg-rose-200" />
            <div>
              ⭐ {items.filter((i) => i.type === "memory").length} Kenangan
            </div>
            <div className="w-[1px] h-4 bg-rose-200" />
            <div>
              👫 {items.filter((i) => i.type === "profile").length} Profil
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 p-4 mb-8 border glass rounded-3xl border-rose-100">
        <div className="flex flex-col items-stretch justify-between gap-4 md:flex-row md:items-center">
          ...
          <div className="relative flex-1">
            <Search
              size={16}
              className="absolute -translate-y-1/2 left-4 top-1/2 text-rose-400"
            />
            <input
              type="text"
          ...
          <div className="flex items-center flex-shrink-0 gap-2">
            <span className="text-xs font-bold text-rose-700 font-body whitespace-nowrap">
              Urutkan:
            </span>
            <select
              value={sortBy}
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-[11px] font-bold transition-all whitespace-nowrap cursor-pointer ${
                  isActive
                    ? "bg-[#2d8c6e] text-white shadow-sm"
      ...
      {loading ? (
        <div className="py-24 text-center">
          ...
          <p className="mt-2 text-sm font-semibold text-rose-500 font-display">
            Merajut momen media kita...
          </p>
        </div>
        ...
        <div className="py-20 text-center border glass rounded-3xl border-rose-100">
          ...
          <h3 className="mb-2 text-xl font-bold font-display text-rose-800">
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-4 text-white transition-opacity duration-300 opacity-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent group-hover:opacity-100">
                ...
                <h4 className="mb-1 text-xs font-bold truncate font-display">
                  ...
                  categoryEmoji[item.category]
                    ? `${categoryEmoji[item.category]} `
                    : ""}
                  {item.type === "memory" && item.mood && moodEmoji[item.mood]
                    ? `${moodEmoji[item.mood]} `
                    : ""}
                  {item.title}
                </h4>
          ...
          >
            <div className="space-y-1.5 flex-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {currentLightboxItem.date && (
                  <span className="text-[10px] text-gray-300 font-body flex items-center gap-1 bg-white/10 px-2 py-0.5 rounded-full">
                    <Calendar size={10} />
                    {format(
                      parseISO(currentLightboxItem.date),
                      "EEEE, d MMMM yyyy",
                      { locale: idLocale },
                    )}
                  </span>
              ...
              <h3 className="text-lg font-bold font-display md:text-xl">
                {currentLightboxItem.type === "documentation" &&
                currentLightboxItem.category &&
                categoryEmoji[currentLightboxItem.category]
                  ? `${categoryEmoji[currentLightboxItem.category]} `
                  : ""}
                {currentLightboxItem.type === "memory" &&
                currentLightboxItem.mood &&
                moodEmoji[currentLightboxItem.mood]
                  ? `${moodEmoji[currentLightboxItem.mood]} `
                  : ""}
      ...
      {showUploadModal && (
        <div
          className="fixed inset-0 flex items-center justify-center p-4 z-55 bg-black/60 backdrop-blur-sm"
          style={{ zIndex: 100 }}
        >
          <div className="w-full max-w-lg p-6 text-gray-800 bg-white border shadow-2xl border-rose-100 rounded-3xl animate-fade-in">
            {/* Modal Header */}
            <div className="flex items-center justify-between mb-5">
              <h2 className="flex items-center gap-2 text-lg font-bold font-display text-rose-800">
              ...
              <button
                ...
                className="p-1 text-gray-400 transition-colors rounded-full cursor-pointer hover:text-rose-500 hover:bg-rose-50"
              >
                <X size={18} />
              </button>
            </div>
            ...
            {/* Error Message */}
            {uploadError && (
              <div className="flex items-center gap-2 p-3 mb-4 text-xs font-semibold text-red-600 border border-red-100 bg-red-50 rounded-xl">
                <span>⚠️</span> {uploadError}
              </div>
            )}

            {/* Route selector: Documentation vs Memory */}
            <div className="flex gap-2 p-1 mb-5 border bg-rose-50/50 rounded-2xl border-rose-100/40">
              <button
                type="button"
            ...
            <div className="space-y-4 max-h-[45vh] overflow-y-auto pr-1">
              {uploadTarget === "documentation" ? (
                <>
                  ...
                  <div>
                    <label className="block mb-1.5 text-xs font-bold text-rose-800 font-body">
                      Tempat yang Dikunjungi *
                    </label>
                    {places.length === 0 ? (
                      <div className="p-3 text-xs border text-rose-500 bg-rose-50/50 rounded-xl border-rose-100/50">
                        Belum ada tempat yang ditandai "Sudah Dikunjungi".
                        Silakan kunjungi halaman Tempat Kenangan untuk
                        menambahkannya!
                      </div>
                    ) : (
                      <select
                        ...
                        className="w-full px-3 py-2 text-xs bg-white border cursor-pointer love-input border-rose-100 rounded-xl focus:outline-none focus:ring-1 focus:ring-rose-300"
                      >
                        {places.map((p) => (
                          <option key={p.id} value={p.id}>
                            {categoryEmoji[p.category || ""]} {p.name} (
                            {p.visited_date
                              ? format(parseISO(p.visited_date), "d MMM yyyy", {
                                  locale: idLocale,
                                })
                              : "Tanggal -"}
                            )
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 text-xs bg-white border love-input border-rose-100 rounded-xl focus:outline-none focus:ring-1 focus:ring-rose-300"
                    />
                  </div>
                </>
              ) : (
              ...
              <div>
                ...
                <div
                  ...
                  className="flex flex-col items-center justify-center p-6 transition-colors border-2 border-dashed cursor-pointer rounded-2xl bg-rose-50/20 hover:bg-rose-50/50 border-rose-200"
                >
                  <Upload
                    size={24}
                    className="mb-2 text-rose-450 animate-bounce"
                  />
                  <p className="text-xs font-bold text-rose-700">
                    Klik untuk mencari file
                  </p>
                  <p className="mt-1 text-[10px] text-rose-400 text-center font-body leading-normal">
                    Format Foto: JPG, PNG (Kompresi Otomatis, max 5MB)
                    <br />
            ...
            {uploading && (
              <div className="flex items-center justify-center gap-2 mt-4 text-xs font-bold text-rose-600 font-display">
                <span className="heart-beat">🌿</span> Mengunggah & mengompresi
                media...
              </div>
            )}
          </div>
