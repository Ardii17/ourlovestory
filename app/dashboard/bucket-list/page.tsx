...
export default function BucketListPage() {
  ...
  async function addItem() {
    ...
    await supabase
      .from("bucket_list")
      .insert([{ ...form, is_completed: false }]);
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-rose-500 font-body">
            Impian-impian yang ingin kalian wujudkan bersama
          </p>
        </div>
        <button
      ...
      {/* Progress */}
      {totalCount > 0 && (
        <div className="p-5 mb-6 border glass rounded-2xl border-rose-100">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <Trophy size={20} className="text-amber-400" />
              <span className="font-bold font-display text-rose-800">
                Progress Impian
              </span>
            </div>
          ...
          <div className="h-3 mb-2 rounded-full bg-rose-100">
            <div
              className="h-3 rounded-full progress-bar"
              style={{ width: `${progress}%` }}
            />
          </div>
          <p className="text-xs text-right text-rose-400 font-body">
            {Math.round(progress)}% terwujud
          </p>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="py-16 text-center glass rounded-3xl">
          ...
          <h3 className="mb-2 text-xl font-display text-rose-700">
            {filter === "completed"
              ? "Belum ada yang terwujud"
              : "Belum ada impian"}
          </h3>
          <p className="mb-4 text-sm text-rose-400 font-body">
            Tulis semua impian yang ingin kalian wujudkan bersama!
          </p>
          {filter === "all" && (
            <button onClick={() => setShowModal(true)} className="btn-rose">
              + Tulis Impian Pertama
            </button>
          )}
        </div>
      ) : (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.is_completed && item.completed_date && (
                      <p className="mt-1 text-xs text-green-500 font-body">
                        🎉 Terwujud pada{" "}
                        {format(parseISO(item.completed_date), "d MMMM yyyy", {
                          locale: idLocale,
                        })}
                      </p>
                    )}
                  ...
                  <button
                    ...
                    className="p-1.5 bg-rose-50 rounded-lg hover:bg-rose-100 transition-colors flex-shrink-0"
                  >
      ...
      {showModal && (
        ...
        >
          <div className="w-full max-w-md p-6 bg-white shadow-2xl rounded-3xl">
            <div className="flex items-center justify-between mb-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Apa yang ingin kalian lakukan bersama?"
                  value={form.title}
                  onChange={(e) =>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Ceritakan lebih detail tentang impian ini..."
